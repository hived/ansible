...
ROOT_DIR=$(cd "$DIR/../" && pwd)
VENDOR_ROLE_DIR="$ROOT_DIR/vendor.roles"
ROLES_REQUIREMNTS_FILE="$ROOT_DIR/roles_requirements.yml"
...
if [ -d "$VENDOR_ROLE_DIR" ]; then
    cd "$VENDOR_ROLE_DIR"
    if [ "$(pwd)" == "$VENDOR_ROLE_DIR" ];then
      echo "Removing current roles in '$VENDOR_ROLE_DIR/*'"
      rm -rf *
    else
      msg_exit "Path error could not change dir to $VENDOR_ROLE_DIR"
    fi
fi

# Install roles
ansible-galaxy install -r "$ROLES_REQUIREMNTS_FILE" --force --no-deps -p "$VENDOR_ROLE_DIR"
